...
class ShowAttMentor(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        self.filter_combo.addItems(list(set(entry['Mentoring Name'] for entry in self.data)))
    ...
    def update_table(self, index):
        ...
        filtered_data = [entry for entry in self.data if entry['Mentoring Name'] == current_text]
...
def show_attendance_ui(file_path):
    app = QApplication([])
    window = ShowAttMentor()
    window.show()
    app.exec_()

file_path = "data/mentor_attendance.csv"
